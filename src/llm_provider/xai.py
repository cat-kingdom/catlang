...
This provider uses langchain_openai.ChatOpenAI with xAI's base URL.
"""
...
class XAIProvider(BaseLLMProvider):
    ...
    xAI provides access to Grok models through an OpenAI-compatible API.
    This provider wraps langchain_openai.ChatOpenAI with xAI's base URL.
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._model: str = config.get("model", config.get("default_model", "grok-beta"))
        self._api_key: Optional[str] = config.get("api_key") or os.getenv("XAI_API_KEY")
    ...
    def initialize(self) -> None:
        ...
        if not self._api_key:
            raise ValueError(
                ...
                "Set it in config['api_key'] or XAI_API_KEY environment variable."
            )
        ...
        try:
            ...
            logger.info(f"xAI provider initialized with model: {self._model}")
            
        except Exception as e:
            logger.error(f"Failed to initialize xAI provider: {e}")
            raise RuntimeError(f"xAI initialization failed: {e}") from e
    ...
    def generate(
        ...
        """Generate a response from xAI synchronously."""
        ...
        try:
            ...
            return GenerationResponse(
                ...
                metadata={"provider": "xai", "base_url": XAI_BASE_URL}
            )
    ...
    def stream(
        ...
        """Stream a response from xAI synchronously."""
        if not self._initialized or not self._client:
            raise RuntimeError("Provider not initialized. Call initialize() first.")
    ...
    async def stream_async(
        ...
        """Stream a response from xAI asynchronously."""
        if not self._initialized or not self._client:
            raise RuntimeError("Provider not initialized. Call initialize() first.")
        ...
        try:
            ...
            logger.error(f"xAI async streaming failed: {e}")
            raise
    ...
    def estimate_cost(
        ...
        Note: xAI pricing may vary. Check https://docs.x.ai for current pricing.
        ...
        """
        # xAI pricing information may not be publicly available
        # Return 0.0 as we can't accurately estimate without official pricing data
        logger.warning(
            "xAI pricing information not available. "
            "Cannot provide accurate cost estimate. "
            "Check https://docs.x.ai for current pricing."
        )
        return 0.0
    
    def validate_credentials(self) -> bool:
        """
        Validate xAI credentials by making a test API call.
    ...
    def get_langchain_client(self) -> ChatOpenAI:
        ...
        Returns:
            The ChatOpenAI client instance configured for xAI
